...
docker_required_variables_env() {
    ...
    if [[ -n "$FE_SERVERS" && -n "$FE_ID" ]]; then
        ...
        if [[ $FE_ID =~ ^[1-9]{1}$ ]]; then
            ...
            doris_error "FE_ID rule error！If FE is the role of Master, please set FE_ID=1, and ensure that all IDs correspond to the IP of the ntnt node."
        fi
        return
    fi
...
_main() {
    ...
    if [[ $RUN_TYPE == "K8S" ]]; then
        ...
        if [ $CURRENT_FE_IS_MASTER == true ]; then
            bash -x /opt/apache-doris/fe/bin/start_fe.sh 
            child_pid=$!
        else
            /opt/apache-doris/fe/bin/start_fe.sh --helper ${MASTER_FE_IP}:${MASTER_FE_EDIT_PORT} &
            child_pid=$!
        fi
    fi
    wait $child_pid
